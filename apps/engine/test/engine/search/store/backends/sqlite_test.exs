defmodule Engine.Search.Store.Backend.SqliteTest do
  ...
  defp start_supervised_store(%Project{} = project, create_fn, update_fn, backend) do
    ...
    start_supervised!(Backends.Sqlite)
  ...
  describe "starting" do
    test "incorporates any indexed files in an empty index", %{project: project, backend: backend} do
      ...
      start_supervised_store(project, &default_create/1, update, backend)
    ...
    test "the updater allows you to delete paths", %{project: project, backend: backend} do
      ...
      assert [structure_entry, _definition_entry] = all_entries(backend)
      assert structure_entry.path == "/path/to/keep.ex"

      assert :error = Backends.Sqlite.structure_for_path("/path/to/delete.ex")
      assert :error = Backends.Sqlite.structure_for_path("/another/path/to/delete.ex")
      assert {:ok, ^kept_structure} = Backends.Sqlite.structure_for_path("/path/to/keep.ex")
    end
  end
