defmodule Engine.Search.IndexerTest do
  ...
  describe "create_index/1" do
    ...
    @tag :tmp_dir
    test "indexes bare projects without treating root/deps as a dependency directory", %{
      tmp_dir: tmp_dir
    } do
      bare_file = write_file!(Path.join(tmp_dir, "bare_file.ex"), "defmodule BareFile do end")
      bare_project = tmp_dir |> Forge.Document.Path.to_uri() |> Project.new()

      patch(Engine, :get_project, fn -> bare_project end)

      assert {:ok, entries} = Indexer.create_index(bare_project)
      assert Enum.any?(entries, &(&1.path == bare_file))
    end
