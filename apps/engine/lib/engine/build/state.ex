defmodule Engine.Build.State do
  ...
  defp project_compile_result({:ok, diagnostics}),
    do: {:success, diagnostic_results(diagnostics)}

  defp project_compile_result({:error, diagnostics}),
    do: {:error, diagnostic_results(diagnostics)}

  defp diagnostic_results(diagnostics) do
    diagnostics
    |> List.wrap()
